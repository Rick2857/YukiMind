...
3. 将 mention 接收者当成事实主体：
   ...
   `@测试成员甲` 是消息接收者，语义主体是“测试成员乙”，不能据此写入测试成员甲的 `person_group`。
...
```text
@测试成员甲 你不玩原神
→ 可以将测试成员甲作为 person_group 候选主体

@测试成员甲 测试成员乙喜欢摄影
→ 测试成员甲是接收者；不得把“测试成员乙喜欢摄影”写给测试成员甲
```
...
|---|---|
| `我喜欢摄影` | 当前发送者 person |
| `喜欢摄影` | 仅在安全省略主语条件下归发送者 |
| `你今天完成了测试任务` | 不得归当前发送者 |
| `Yuki 是 CI runner` | 不得归当前发送者；可产生 self candidate |
| `@测试成员甲 测试成员乙喜欢摄影` | 不得归测试成员甲 |
| `@测试成员甲 你不玩原神` | metadata 和语义均成立时可归测试成员甲 person_group |
| `测试成员乙是这个群的群主` | 优先 group；不得降级到发送者 |
| 普通姓名无法唯一匹配 | 不猜人物 |
