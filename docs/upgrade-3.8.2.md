...
目标数据库版本为 **0059**。从 0051 升级会执行 0052–0059 迁移；禁止直接 stamp 跳过迁移。
更旧数据库先核对 [3.8.1 升级要求](https://github.com/Rick2857/YukiMind/blob/v3.8.2/docs/upgrade-3.8.1.md)。
...
新安装需要选择并登录 NapCat；填好网关配置不等于已经完成 QQ 登录。公开部署不自动安装或
启动 SnowLuma，已经独立取得上游软件和相应权利的操作者只能按[手工接入边界](deployment/snowluma.md)连接。
...
[持久环境说明](https://github.com/Rick2857/YukiMind/blob/v3.8.2/docs/operations/persistent-environment.zh-CN.md)。
...
回退镜像必须支持当前数据库 schema；0052–0059 包含语音转写、任务回执与运行预算等变化，
旧版二进制不一定兼容。0058 会删除无法可靠解析的旧 Web 搜索缓存，0059 会保留已有最大计数
并暂停语义不明确的旧任务；这些数据决策不能由 Alembic downgrade 自动还原，因此必须依赖升级前
的一致备份进行灾难恢复。
保留新消息、家目录、artifact 和执行回执；没有数据损坏证据时不用旧库覆盖新数据，
也不能通过清空上下文、记忆或重建 embedding 掩盖问题。历史状态链及证据恢复不由本版自动完成。

完整变更和残余风险见 [3.8.2 说明](https://github.com/Rick2857/YukiMind/blob/v3.8.2/docs/releases/v3.8.2.md)。
