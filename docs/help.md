...
Yuki 3.8 只支持 canonical runtime，当前源码 Alembic head 为 `0059`，Plugin API 为 `2.0`。永久 Yuki、
...
两个安装器默认安装 `3.8.2`。引导配置会询问主模型、可选 Flash/Embedding/Web/Vision、QQ
...
公开 Compose 和安装器只自动管理 NapCat。同一 QQ 只能有一条活动连接；重复连接拒绝新的，
不会自动挤掉旧连接。

手工接入其他兼容 Provider 时，切换同一 QQ 必须按以下顺序操作：

1. 停止并移除旧 Provider。
2. 确认旧连接已从 Registry 注销。
3. 按其自身条款和文档启动目标 Provider。
4. 验证新连接沿用原 Presence。

停止失败时不会启动新 Provider。切换不改变 Conversation、Memory 或 RouteGeneration。

源码仍保留 SnowLuma 的 OneBot 兼容适配，但公开发行不提供其镜像、Compose 服务、下载或向导
自动化。只有已经独立取得上游软件并确认相应权利的操作者，才可按
[SnowLuma 手工接入边界](deployment/snowluma.md)连接。
...
全新 3.8.2 数据库执行到 `0059`。历史 bridge 只接受已经完成 canonical v2 的旧 `0048`；
已有 3.8.1 数据库继续顺序执行迁移，不能用 `stamp` 跳过。
...
`0049` 不提供 downgrade；0052–0059 还包含语音转写、任务回执和运行预算变化，其中 0058 删除
无法可靠解析的旧 Web 搜索缓存，0059 保留已有最大计数并暂停语义不明确的旧任务。生产失败
时仍应恢复完整快照，不能手工
...
```bash
docker compose --profile napcat ps --all
...
[发布说明](releases/v3.8.2.md)。正式镜像与 Release 只能由通过 Quality、迁移矩阵、release
