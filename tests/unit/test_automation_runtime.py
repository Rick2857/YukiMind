...
async def test_superuser_script_persists_explicit_person_not_creator(database) -> None:
    ...
    row = await service.create(
        _script_to("10002"),
        inbound=_superuser_inbound("10002"),
        conversation_key="private:9000",
    )
    ...
    async with database.sessions() as session:
        assert await person_id_for(session, "10002") == row.canonical_target_person_id
        assert await person_id_for(session, "9000") == row.canonical_creator_person_id
        assert await person_id_for(session, "10002") != await person_id_for(session, "9000")
...
async def test_multiple_distinct_send_targets_are_rejected(database) -> None:
    ...
    with pytest.raises(ValueError, match="一个永久发送目标"):
        await service.create(
            _script_to("10002", extra="10003"),
            inbound=_superuser_inbound("10002", "10003"),
            conversation_key="private:9000",
        )
...
async def test_update_switches_to_same_person_alias(database) -> None:
    ...
    async with database.sessions() as session:
        target = await person_id_for(session, "10002")
    ...
    inbound = _superuser_inbound("10002", "10004")
    row = await service.create(
        _script_to("10002"),
        inbound=inbound,
        conversation_key="private:9000",
...
async def test_disabled_binding_fails_while_space_target_remains_canonical(database) -> None:
    ...
    async with database.sessions() as session, session.begin():
        binding = await session.scalar(
            select(IdentityBindingModel).where(IdentityBindingModel.external_account_id == "10002")
        )
