...
async def test_group_directory_miss_refreshes_without_group_message(database: Database) -> None:
    ...
    await UserProfileRepository(database).observe(
        user_id="1001", nickname="测试成员丁", group_id="2001"
    )
    ...
    service._call = AsyncMock(
        return_value=[
            {"group_id": 2001, "group_name": "示例群组"},
            {"group_id": 9988776655, "group_name": "未登记群"},
        ]
    )
    assert await service.contacts("space", "示例群组", exact=True) == [
        {"target_id": space_id, "display_name": "示例群组", "kind": "space"}
    ]
    ...
    assert await MemoryReadScopeResolver(database).groups_named("99999", "示例群组") == ()
