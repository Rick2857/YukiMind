...
def test_release_identity_matches_all_version_surfaces() -> None:
    ...
    for path in (
        ...
        assert "0059" in path.read_text(encoding="utf-8")
    assert "## 3.8.2 - Release candidate" not in changelog
...
def test_production_and_development_compose_are_separated() -> None:
    ...
    assert "ghcr.io/rick2857/yukimind:${YUKI_VERSION:?missing}" in production
    assert "ghcr.io/rick2857/yukimind-genie-tts-worker:${YUKI_VERSION:?missing}" in production
    ...
    assert 'profiles: ["snowluma"]' not in production
    assert (
        "mlikiowa/napcat-docker@"
        "sha256:1336a777f9a4f1f8cb89fef42f7548deacd3645919a067a50df5b66b5e77390e" in production
    )
    assert "motricseven7/snowluma" not in production
    assert "SNOWLUMA_" not in production
    assert "snowluma-data" not in production
    ...
    worker_lock = (ROOT / "services/genie_tts_worker/uv.lock").read_text(encoding="utf-8")
    worker_dockerfile = (ROOT / "services/genie_tts_worker/Dockerfile").read_text(encoding="utf-8")

    assert "anchore/sbom-action@e22c389904149dbc22b58101806040fa8d37a610" in workflow
    assert "yukimind-${{ needs.validate.outputs.version }}.spdx.json" in workflow
    assert "yukimind-genie-tts-worker-${{ needs.validate.outputs.version }}.spdx.json" in workflow
    assert 'name = "e2k"' not in worker_lock
    assert "/usr/share/licenses/yukimind" in worker_dockerfile
