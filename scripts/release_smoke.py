...
def validate_production_compose(deploy_directory: Path, version: str, compose: Compose) -> None:
    ...
    expected = {
        "bot": f"ghcr.io/rick2857/yukimind:{version}",
        "genie-tts-worker": f"ghcr.io/rick2857/yukimind-genie-tts-worker:{version}",
    }
    ...
    if napcat["image"] != (
        ...
        "sha256:1336a777f9a4f1f8cb89fef42f7548deacd3645919a067a50df5b66b5e77390e"
    ):
        raise SmokeError("NapCat image does not resolve to the pinned upstream image")
    required_mounts = {
        "bot": {
            "/app/data",
            "/app/config",
...
def verify_guided_setup(deploy_directory: Path, version: str) -> None:
    image = f"ghcr.io/rick2857/yukimind:{version}"
